...
import { Box, Code, PanelRightOpen, PanelRightClose, ChevronLeft, ChevronRight, Layers, Rotate3d, GraduationCap, AlertTriangle, X, FileCode } from 'lucide-react';
...
function AppContent() {
  ...
  const [activeTab, setActiveTab ] = useState<'designer' | 'threed' | 'didactic'>('designer');
  const [showAlert, setShowAlert ] = useState(false);

  // Check alert visibility status
  useEffect(() => {
    const dismissedUntil = localStorage.getItem('vibe_coded_alert_dismissed_until');
    const now = Date.now();
  ...
  const handleDismissAlert = () => {
    ...
    localStorage.setItem('vibe_coded_alert_dismissed_until', (Date.now() + twentyFourHours).toString());
    setShowAlert(false);
  };
    ...
    <div className="h-screen w-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans overflow-hidden relative">
        ...
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-lg transition-all duration-300">
          <div className="bg-zinc-900/95 border border-amber-500/30 text-amber-200/95 shadow-[0_10px_40px_rgba(0,0,0,0.8)] rounded-xl p-4 backdrop-blur-md flex items-start gap-3.5">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="text-xs font-bold text-amber-450 tracking-wider uppercase select-none">Vibe-Coded Warning</h4>
              <p className="text-[11.5px] text-zinc-305 leading-relaxed mt-1">
                This site was mainly <strong>warning-coded</strong> (vibe-coded) and may contain <strong>AI slop</strong>. Explore with critical thinking! 🤖✨
              </p>
            </div>
            <button 
      ...
      <header className="h-14 border-b border-zinc-800/50 flex items-center justify-between px-6 shrink-0 bg-zinc-900/40 relative z-10 backdrop-blur-sm">
        <div className="flex items-center">
          ...
          <h1 className="font-semibold text-[13px] tracking-widest text-zinc-300 uppercase select-none hidden sm:inline">Neural Network Architect</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          {selectedNodeId !== null && activeTab === 'designer' && (
            <button 
              ...
              title={isSidebarOpen ? "Hide configuration sidebar" : "Show configuration sidebar"}
            >
              {isSidebarOpen ? (
                <>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar relative bg-[#0a0a0c]">
          ...
          {!isSidebarOpen && selectedNodeId !== null && activeTab === 'designer' && (
            <button
            ...
            >
              <div className="flex flex-col items-center gap-1">
                ...
                <span className="text-[9px] uppercase tracking-widest font-bold font-sans [writing-mode:vertical-lr] text-zinc-500 group-hover:text-zinc-350 select-none">PARAMS</span>
              </div>
            </button>
          )}
        </main>
        ...
        <aside className={cn(
          ...
          isSidebarOpen ? "w-80 opacity-100" : "w-0 opacity-0 pointer-events-none border-l-0 overflow-hidden"
        )}>

...
export function Inspector() {
  const { layers, inputShape, setInputShape, selectedNodeId, updateLayer } = useNetwork();
  ...
  if (selectedNodeId === 'input') {
    return (
      <div className="p-6 h-full flex flex-col overflow-y-auto">
        <h2 className="text-[10px] font-bold tracking-widest text-cyan-500 uppercase mb-6">Configure Dimensions</h2>
        
        <div className="space-y-6">
          <div>
            <label className="flex items-center justify-between cursor-pointer select-none border-b border-zinc-800 pb-3 mb-4">
              <span className="text-xs font-semibold text-zinc-300">Dimension Type</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-zinc-500 mb-1.5 font-mono">Channels (C)</label>
              <input 
                type="number"
                value={inputShape.c}
                onChange={(e) => setInputShape({ ...inputShape, c: Math.max(1, parseInt(e.target.value) || 1) })}
            ...
            {inputShape.d !== undefined ? (
              <div>
                <label className="block text-xs font-medium text-zinc-500 mb-1.5 font-mono">Depth (D)</label>
                <input 
                  type="number"
                  value={inputShape.d}
                  onChange={(e) => setInputShape({ ...inputShape, d: Math.max(1, parseInt(e.target.value) || 1) })}
                  className="w-full bg-zinc-900 border border-zinc-800 focus:border-cyan-500 rounded-lg px-2.5 py-1.5 text-xs text-zinc-200 font-semibold outline-none"
                />
              </div>
            ) : (
              <div className="opacity-30 pointer-events-none">
                <label className="block text-xs font-medium text-zinc-600 mb-1.5 font-mono">Depth (N/A)</label>
            ...
            <div>
              <label className="block text-xs font-medium text-zinc-500 mb-1.5 font-mono">Height (H)</label>
              <input 
                type="number"
                value={inputShape.h}
                onChange={(e) => setInputShape({ ...inputShape, h: Math.max(1, parseInt(e.target.value) || 1) })}
                className="w-full bg-zinc-900 border border-zinc-800 focus:border-cyan-500 rounded-lg px-2.5 py-1.5 text-xs text-zinc-200 font-semibold outline-none"
              />
            </div>

            <div>
              <label className="block text-xs font-medium text-zinc-500 mb-1.5 font-mono">Width (W)</label>
              <input 
                type="number"
                value={inputShape.w}
                onChange={(e) => setInputShape({ ...inputShape, w: Math.max(1, parseInt(e.target.value) || 1) })}
                className="w-full bg-zinc-900 border border-zinc-800 focus:border-cyan-500 rounded-lg px-2.5 py-1.5 text-xs text-zinc-200 font-semibold outline-none"
              />
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-zinc-800/60">
            ...
            <div className="grid grid-cols-1 gap-2">
              ...
              >
                ImageNet <span className="text-[10px] text-zinc-500 font-mono ml-1.5">(3 × 224 × 224)</span>
              </button>
              <button
                type="button"
                ...
                className="px-2.5 py-2 text-left text-xs font-medium text-zinc-400 hover:text-cyan-400 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded-lg transition-all"
              >
                MNIST/Fashion <span className="text-[10px] text-zinc-500 font-mono ml-1.5">(1 × 28 × 28)</span>
              </button>
              <button
                type="button"
                ...
                className="px-2.5 py-2 text-left text-xs font-medium text-zinc-400 hover:text-cyan-400 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded-lg transition-all"
              >
                Spectrogram <span className="text-[10px] text-zinc-500 font-mono ml-1.5">(1 × 128 × 256)</span>
              </button>
              <button
                type="button"
                ...
                className="px-2.5 py-2 text-left text-xs font-medium text-zinc-400 hover:text-cyan-400 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded-lg transition-all"
              >
                3D Imaging <span className="text-[10px] text-zinc-500 font-mono ml-1.5">(3 × 32 × 64 × 64)</span>
              </button>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="p-6 h-full flex flex-col overflow-y-auto">
      <h2 className="text-[10px] font-bold tracking-widest text-zinc-500 uppercase mb-6">Inspect Layer</h2>
      
      <div className="space-y-6">
        <div>
          <label className="block text-xs text-zinc-500 mb-1.5 font-medium">Layer Name</label>
          <input 
            type="text" 
        ...
        {(selectedNode.type === 'conv2d' || selectedNode.type === 'conv3d') && (
          <div className="space-y-4">
            <h3 className="text-xs font-semibold text-zinc-300 border-b border-zinc-800 pb-2 mb-4">Convolution Parameters</h3>
            ...
            <div>
              ...
              <input 
                ...
                onChange={(e) => handleParamChange('filters', parseInt(e.target.value) || 1)}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-zinc-400 mb-1.5">Kernel Size</label>
                <input 
                  type="number" 
              ...
              <div>
                <label className="block text-xs text-zinc-400 mb-1.5">Stride</label>
                <input 
                  type="number" 
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1.5">Padding</label>
        ...
        {(selectedNode.type === 'relu' || selectedNode.type === 'flatten' || selectedNode.type === 'batchnorm2d' || selectedNode.type === 'batchnorm3d' || selectedNode.type === 'group') && (
          <div className="pt-4 text-center">
            <span className="text-xs text-zinc-500">
              {selectedNode.type === 'group' ? 'No configurable parameters for this group.' : 'No configurable parameters for this layer.'}
            </span>
          </div>
        )}

        {(selectedNode.type === 'maxpool2d' || selectedNode.type === 'maxpool3d') && (
          <div className="space-y-4">
            <h3 className="text-xs font-semibold text-zinc-300 border-b border-zinc-800 pb-2 mb-4">Pooling Parameters</h3>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-zinc-400 mb-1.5">Pool Size</label>
                <input 
                  type="number" 
                  value={selectedNode.params?.poolSize || 2}
                  onChange={(e) => handleParamChange('poolSize', parseInt(e.target.value) || 1)}
                  className="w-full bg-zinc-900 border border-zinc-800 focus:border-indigo-500 rounded-md px-3 py-2 text-sm text-zinc-200 outline-none transition-colors"
                />
              </div>
              <div>
        ...
        {selectedNode.type === 'linear' && (
          <div className="space-y-4">
            <h3 className="text-xs font-semibold text-zinc-300 border-b border-zinc-800 pb-2 mb-4">Linear/Dense Parameters</h3>
            <div>
              <label className="block text-xs text-zinc-400 mb-1.5">Out Features (Units)</label>
              <input 
                type="number" 
                value={selectedNode.params?.outFeatures || 128}
                onChange={(e) => handleParamChange('outFeatures', parseInt(e.target.value) || 1)}
                className="w-full bg-zinc-900 border border-zinc-800 focus:border-indigo-500 rounded-md px-3 py-2 text-sm text-zinc-200 outline-none transition-colors"
              />
            </div>
          </div>
        ...
        {selectedNode.type === 'dropout' && (
          <div className="space-y-4">
            <h3 className="text-xs font-semibold text-zinc-300 border-b border-zinc-800 pb-2 mb-4">Dropout Parameters</h3>
            <div>
              <label className="block text-xs text-zinc-400 mb-1.5">Rate (Probability)</label>
              <input 
                type="number" 
                step="0.1"
                min="0"

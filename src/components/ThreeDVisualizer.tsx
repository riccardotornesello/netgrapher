...
export function ThreeDVisualizer() {
  const { layers, inputShape, selectedNodeId, setSelectedNodeId } =
    useNetwork();
  ...
  traceLayers.forEach((tl, i) => {
    ...
    if (i > 0) {
      ...
      if (
        ...
        const prevD =
          (scaleHW(prevTL.outShape.d ? prevTL.outShape.d : 1) * 0.4) / 2;
        ...
        corners.forEach((cItem, ci) => {
          ...
          const pitchRad = (pitch * PI) / 180;
          const avgZ =
            ((pt1.z + pt2.z) / 2) * cos(yawRad) +
            ((pt1.x + pt2.x) / 2) * sin(yawRad);
          ...
          polygonItems.push({
            ...
            stroke:
              tl.node && selectedNodeId === tl.node?.id ? "#06b6d4" : "#3f3f46",
            strokeWidth: 1.0,
            strokeDasharray: "3,4",
            opacity: tl.node && selectedNodeId === tl.node?.id ? 0.7 : 0.35,
          });
        });
      }
    }
    ...
    if (
      ...
      if (
        ...
        (tl.node.type === "conv2d" ||
          tl.node.type === "conv3d" ||
          tl.node.type === "maxpool2d" ||
          tl.node.type === "maxpool3d")
      ) {
        ...
        const stepIndex =
          Math.floor((animationTick / 360) * totalSteps) % totalSteps;
        ...
        const sweepX = 0.15 + 0.7 * (colIndex / (gridCols - 1));
        const sweepY = 0.15 + 0.7 * (rowIndex / (gridRows - 1));
        ...
        const pitchRad = (pitch * PI) / 180;
        const avgZ =
          ((pt1.z + targetPt.z) / 2) * cos(yawRad) +
          ((pt1.x + targetPt.x) / 2) * sin(yawRad);
        ...
        funnelPts.forEach((fp) => {
          polygonItems.push({
            type: "line",
            points: [{ x: prevX + srcD / 2, y: fp.y, z: fp.z }, targetPt],
            avgZ: avgZ + 50,
            stroke: "#06b6d4",
            strokeWidth: 0.9,
            opacity: 0.65,
  ...
  const selectedNodeIndex = traceLayers.findIndex(
    (tl) => tl.node?.id === selectedNodeId,
  );
  const selectedTrace =
      ...
      <div className="flex-1 bg-zinc-950/40 rounded-2xl border border-zinc-800/40 backdrop-blur-md flex flex-col relative overflow-hidden h-[420px] xl:h-auto min-h-[380px]">
        ...
        <div className="absolute top-4 left-4 z-20 flex flex-wrap gap-1.5 items-center">
          <button
            ...
            className={cn(
              ...
              dragMode === "rotate"
                ? "bg-cyan-950/50 border border-cyan-800/60 text-cyan-400"
        ...
        >
          ...
          >
            ...
            {polygonItems.map((item, idx) => {
              ...
              if (
                (item.type === "face" || item.type === "kernel_face") &&
                item.projectedPoints
              ) {
                const pointsStr = item.projectedPoints
                  .map((p) => `${p.x},${p.y}`)
                  .join(" ");
                const isSelectedFace =
                  item.layerId && selectedNodeId === item.layerId;
                  ...
                  <g key={`face-g-${idx}`} className="group/face">
                    <polygon
                      ...
                      className={cn(
                        ...
                        isSelectedFace
                          ? "stroke-[2px] filter drop-shadow-[0_0_8px_rgba(6,182,212,0.3)]"
                          : "hover:fill-opacity-50",
                      )}
                    />

                    {/* Hover tooltip hint */}
                    {item.tooltipText && <title>{item.tooltipText}</title>}
                  </g>
                );
              }
        ...
        <div className="absolute bottom-4 right-4 z-20 flex gap-1.5 p-1.5 bg-zinc-900/75 border border-zinc-800/80 rounded-xl backdrop-blur-sm shadow-xl overflow-x-auto max-w-[85%] select-none custom-scrollbar">
          {traceLayers.map((tl, index) => {
            const isInput = tl.node === null;
            const isClickActive = isInput
              ? selectedNodeId === null
              : selectedNodeId === tl.node?.id;
            return (
              <button
                key={`crumb-${index}`}
                onClick={() =>
                  setSelectedNodeId(isInput ? null : tl.node?.id || null)
                }
                className={cn(
              ...
              >
                <span>{index}</span>
                <span className="opacity-60 max-w-[50px] truncate">
                  {isInput ? "INPUT" : tl.node?.name}
                </span>
              </button>
            );
          })}
      ...
      <div className="w-full xl:w-96 shrink-0 flex flex-col gap-4">
          ...
          {!selectedTrace.isCompatible && (
            <div className="mt-4 p-3 bg-red-950/40 ring-1 ring-red-920 border-l-2 border-red-500 rounded-xl flex gap-2.5 items-start">
              ...
              <div>
                ...
                <p className="text-[11px] text-red-400/90 leading-relaxed mt-0.5">
          ...
          <div className="flex-1 bg-zinc-900/40 border border-zinc-800/40 rounded-2xl p-5 backdrop-blur-md shadow-xl flex flex-col gap-4">
            ...
            <div className="flex gap-4 items-start p-3 bg-zinc-950/50 border border-zinc-850 rounded-xl">
              ...
              <div className="flex-1 overflow-hidden">
                <div className="text-[9.5px] uppercase font-bold tracking-widest text-zinc-500">
                ...
                <div
                  className="text-[10px] text-zinc-500 font-mono mt-1 break-words line-clamp-2"
                  title={stats.parameterFormula}
                >
              ...
              <div className="font-mono text-[10.5px] bg-zinc-950 p-2.5 rounded-lg border border-zinc-900 leading-relaxed flex flex-col gap-1.5 text-zinc-400">
                {stats.dimensionFormulaD && (
                  <div className="flex flex-col gap-0.5 border-b border-zinc-900 pb-1.5">
                    <span className="text-[8.5px] font-sans text-indigo-400 uppercase tracking-wider font-bold">
                      DEPTH (D):
                    </span>
        ...
        {selectedTrace.node === null && (
          <div className="bg-zinc-900/40 border border-zinc-800/40 rounded-2xl p-5 backdrop-blur-md shadow-xl flex-1 flex flex-col gap-3.5">
            <h4 className="text-[10px] uppercase font-bold text-emerald-400 tracking-wider">
              Initial Source Data
            </h4>
            <p className="text-[11.5px] leading-relaxed text-zinc-400 font-sans">
              This block defines the initial shape of the data propagated
              through the network. You can alter these values using the
              ImageNet, MNIST, or Spectrogram shape presets in the header above
              or by editing the parameters in the canvas sidebar.
            </p>
            ...
            <div className="p-3 bg-emerald-950/10 border border-emerald-900/25 rounded-xl text-[10.5px] leading-relaxed text-emerald-400/90 mt-auto flex items-start gap-2">
              ...
              <span>
                Ready for training. The model calculates a reduced output with

...
export const info: LayerDescription = {
  ...
  concept:
    "Slides a set of learnable spectral filters across the width and height of a 2D spatial input map. Each filter computes a localized dot product (MAC) to produce a 2D activation grid featuring extracted local characteristics, such as edges, textures, or composite object shapes.",
  keyTakeaways: [
  ...
  proTips:
    "Choose smaller filters (e.g., 3x3) over larger filters (e.g., 7x7 or 11x11). Stacking multiple 3x3 convolutional layers provides the same effective receptive field with fewer parameters and inserts more non-linear activations.",
  forwardEquation:
    "y_{c, i, j} = b_c + \\sum_{m=0}^{C_{in}-1} \\sum_{h=0}^{K-1} \\sum_{w=0}^{K-1} w_{c, m, h, w} \\cdot x_{m, i \\cdot s + h, j \\cdot s + w}",
  derivativeEquation:
    "\\frac{\\partial L}{\\partial w} = \\sum_{patches} x \\circ \\frac{\\partial L}{\\partial y}, \\quad \\frac{\\partial L}{\\partial x} = \\text{transposed\\_conv}(w, \\frac{\\partial L}{\\partial y})",
  sizeFormulaHTML:
    "H_{out} = \\lfloor\\frac{H_{in} + 2P - K}{S}\\rfloor + 1, \\quad W_{out} = \\lfloor\\frac{W_{in} + 2P - K}{S}\\rfloor + 1",
  parameterFormula:
    "\\text{Parameters} = (K \\cdot K \\cdot C_{in}) \\cdot C_{out} + C_{out}",
  flopFormula:
    "\\text{FLOPs} = 2 \\cdot H_{out} \\cdot W_{out} \\cdot C_{out} \\cdot (K \\cdot K \\cdot C_{in})",
...
export const InteractiveSimulator = () => {
  ...
  for (let kr = 0; kr < 3; kr++) {
    for (let kc = 0; kc < 3; kc++) {
      const inVal =
        convInputGrid[currentConvCoord.r + kr][currentConvCoord.c + kc];
        ...
        <div className="bg-zinc-900/75 p-4 rounded-xl border border-zinc-850 font-mono text-xs text-zinc-300">
          <div className="text-zinc-500 text-[10px] uppercase font-bold tracking-wide mb-1 flex justify-between">
            ...
            <span className="text-cyan-400">
              Position r:{currentConvCoord.r}, c:{currentConvCoord.c}
            </span>
          </div>
          <div className="bg-zinc-950 p-2 text-center rounded-lg border border-zinc-900 text-[11px] truncate flex flex-wrap justify-center gap-1">
            {convSumSteps.map((s, idx) => (
              <span
                key={idx}
                className={idx === 4 ? "text-cyan-400 font-bold" : ""}
              >
                {s} {idx < 8 ? "+" : ""}
              </span>
            ))}
          ...
          <div className="text-right mt-2 text-xs flex justify-between items-center pt-2 border-t border-zinc-900">
            <span className="text-zinc-500 font-sans">
              Accumulated dot product sum
            </span>
            <span className="font-bold text-sm text-emerald-400">
              Result: {convTotalValue}
            </span>
          </div>
        </div>
            ...
            {convInputGrid.map((row, rIdx) =>
              row.map((val, cIdx) => {
                const isKernelFocussed =
                  rIdx >= currentConvCoord.r &&
                  rIdx < currentConvCoord.r + 3 &&
                  cIdx >= currentConvCoord.c &&
                  cIdx < currentConvCoord.c + 3;
                return (
                  <div
                    key={`in-${rIdx}-${cIdx}`}
                    className={cn(
            ...
            <div className="grid grid-cols-3 gap-0.5 border border-cyan-800/40 p-1 rounded-lg bg-cyan-950/20 shadow-md">
              {convKernel.map((row, r) =>
                row.map((v, c) => (
                  <div
                    key={`k-${r}-${c}`}
                    className="w-5.5 h-5.5 rounded flex items-center justify-center text-[9px] font-mono text-cyan-300 font-bold border border-cyan-950"
                  >
                    {v > 0 ? `+${v}` : v}
                  </div>
                )),
          ...
          <div className="grid grid-cols-3 gap-0.5 border border-emerald-900/30 p-1 rounded-lg bg-zinc-950 shadow-md">
            {Array.from({ length: 9 }).map((_, stepIdx) => {
              ...
              for (let kr = 0; kr < 3; kr++) {
                for (let kc = 0; kc < 3; kc++) {
                  score +=
                    convInputGrid[cellCoord.r + kr][cellCoord.c + kc] *
                    convKernel[kr][kc];
                }
              }

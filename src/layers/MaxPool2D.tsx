...
export const info: LayerDescription = {
  ...
  category: "Pooling",
  concept:
    "Slides a 2D window across spatial activations and keeps only the maximum value inside each patch. This downsamples the representations, reducing height and width dimensions.",
  keyTakeaways: [
  ...
  proTips:
    "Recent architectures (e.g., ConvNeXt, ResNet-D) often replace maxpooling with strided convolutions. Strided convolutions let the network learn *how* to downsample instead of using a fixed max function.",
  forwardEquation:
    "y_{c, i, j} = \\max_{h, w} \\{ x_{c, i \\cdot s + h, j \\cdot s + w} \\}",
  derivativeEquation:
    "\\frac{\\partial L}{\\partial x_{idx}} = \\frac{\\partial L}{\\partial y} \\quad (\\text{gradient goes entirely to the index of maximum value})",
  sizeFormulaHTML:
    "H_{out} = \\lfloor\\frac{H_{in} - K}{S}\\rfloor + 1, \\quad W_{out} = \\lfloor\\frac{W_{in} - K}{S}\\rfloor + 1",
  parameterFormula: "\\text{Parameters} = 0",
  flopFormula:
    "\\text{FLOPs} = H_{out} \\cdot W_{out} \\cdot C_{in} \\cdot (K \\cdot K - 1) \\quad (\\text{Compares window cells})",
    ...
    <div className="w-full flex flex-col lg:flex-row gap-8 items-center">
      <div className="flex-1 flex flex-col gap-4">
        ...
        <p className="text-[11.5px] leading-relaxed text-zinc-400">
          Selects exclusively the absolute peak (Maximum) from the current
          localized window coordinate. Observe the reduction in frame dimension:
        </p>
            ...
            {(() => {
              ...
              return (
                <div className="flex items-center justify-between">
                  <span>
                    Scanning patch {"->"} [ {vals.join(", ")} ]
                  </span>
                  <span className="text-cyan-400 font-bold ml-1">
                    Max value found: {Math.max(...vals)}
                  </span>
                </div>
              );
            })()}
            ...
            {poolInputGrid.map((row, r) =>
              row.map((val, c) => {
                ...
                const patchVals = isActivePatch
                  ...
                  : [];
                const isMaxInPatch =
                  isActivePatch && val === Math.max(...patchVals);

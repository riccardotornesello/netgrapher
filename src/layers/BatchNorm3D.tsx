...
export const info: LayerDescription = {
  ...
  concept:
    "Extends 2D Batch Normalization to apply across volumetric datasets. Normalizes activations per channel across batch, depth, height, and width coordinates.",
  keyTakeaways: [
  ...
  proTips:
    "3D Batch Normalization requires larger batch sizes to compute clean batch statistics. If you are forced to use a small batch size (e.g., 1 or 2 due to GPU memory limits), try Group Normalization (GroupNorm) instead.",
  forwardEquation:
    "\\hat{x}_{c} = \\frac{x_c - \\mu_{c,B}}{\\sqrt{\\sigma^2_{c,B} + \\epsilon}}, \\quad y_{c} = \\gamma_c \\hat{x}_c + \\beta_c",
  derivativeEquation:
    "\\text{Adapts 3D parameters by pooling volumetric local gradients}",
  sizeFormulaHTML:
    "D_{out} = D_{in}, \\quad H_{out} = H_{in}, \\quad W_{out} = W_{in}",
  parameterFormula: "\\text{Parameters} = 4 \\cdot C",
  flopFormula:
    "\\text{FLOPs} = 4 \\cdot (C \\cdot D_{in} \\cdot H_{in} \\cdot W_{in})",

...
export const info: LayerDescription = {
  ...
  concept: 'Connects every single input node to every single output node. Performs a matrix multiplication of inputs by a learnable weight matrix, then adds a bias vector.',
  keyTakeaways: [
  ...
  proTips: 'Because fully connected layers contain so many parameters, they are highly prone to overfitting. Guard them with Dropout or replace them with Global Average Pooling (GAP) before classifier projections.',
  forwardEquation: '\\mathbf{y} = \\mathbf{W} \\mathbf{x} + \\mathbf{b}',
  derivativeEquation: '\\frac{\\partial L}{\\partial \\mathbf{x}} = \\mathbf{W}^T \\frac{\\partial L}{\\partial \\mathbf{y}}, \\quad \\frac{\\partial L}{\\partial \\mathbf{W}} = \\frac{\\partial L}{\\partial \\mathbf{y}} \\mathbf{x}^T',
  sizeFormulaHTML: '\\text{Shape}_{out} = [\\text{Batch}, \\text{Out\\_Features}]',
  parameterFormula: '\\text{Parameters} = \\text{In\\_Features} \\cdot \\text{Out\\_Features} + \\text{Out\\_Features}',
  flopFormula: '\\text{FLOPs} = 2 \\cdot \\text{In\\_Features} \\cdot \\text{Out\\_Features}',
...
export const InteractiveSimulator = () => {
  const [linInputWeights, setLinInputWeights] = useState<number[]>([0.5, -0.8, 1.2, -0.3]);
    ...
    <div className="w-full flex flex-col lg:flex-row gap-8 items-center">
      <div className="flex-1 flex flex-col gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-mono text-zinc-400 uppercase tracking-widest">Neuron Connection Playground</span>
        </div>

        <p className="text-[11.5px] leading-relaxed text-zinc-400">
          Calculates a global dot product for every neuron. Adjust the weights index representing connections from the input vector to the target layer node unit:
        </p>

        <div className="grid grid-cols-4 gap-2 text-center">
          {linInputWeights.map((w, wIdx) => (
            <div key={wIdx} className="bg-zinc-900/50 p-2 rounded-xl border border-zinc-850">
              <span className="text-[8.5px] text-zinc-500 block font-mono">Weight {wIdx}</span>
              <input
                type="number"
                step="0.1"
                value={w}
        ...
        <div className="bg-zinc-900/60 p-3.5 rounded-xl border border-zinc-850 font-mono text-xs text-zinc-300">
          ...
          <div className="mt-2 text-[10.5px] flex justify-between items-center text-zinc-350 flex-wrap gap-1.5">
            <span>Log: (1.0×{linInputWeights[0]}) + (2.0×{linInputWeights[1]}) + (-1.0×{linInputWeights[2]}) + (3.0×{linInputWeights[3]}) + 0.5 [bias]</span>
            <span className="text-emerald-400 font-bold">
              Result: {(1.0*linInputWeights[0] + 2.0*linInputWeights[1] - 1.0*linInputWeights[2] + 3.0*linInputWeights[3] + 0.5).toFixed(2)}
            </span>
          </div>
        </div>
      </div>
      ...
      <svg className="w-48 h-40 border border-zinc-855 rounded-xl bg-zinc-950 pointer-events-none p-2 shrink-0">
        <g transform="translate(10, 10)">
          {[20, 50, 80, 110].map((coordY, idx) => (
            <circle key={`n1-${idx}`} cx="20" cy={coordY} r="6" fill="#4f46e5" />
          ))}
          ...
          {[25, 65, 105].map((coordYOut, idx) => (
            <circle key={`n2-${idx}`} cx="120" cy={coordYOut} r="6" fill="#06b6d4" className="animate-pulse" />
          ))}
        </g>
      </svg>
    </div>
...
export class LinearLayer extends Layer {
  ...
  checkCompatibility(inputShape: ImageShape): CompatibilityResult {
    if (inputShape.h > 1 || inputShape.w > 1 || inputShape.d !== undefined) {
      const dims = inputShape.d !== undefined 

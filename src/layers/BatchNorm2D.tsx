...
export const info: LayerDescription = {
  ...
  concept: 'Normalizes activations across the batch dimension per channel. Subtracts the mean and divides by the standard deviation, then applies a learnable scale (gamma) and shift (beta).',
  keyTakeaways: [
  ...
  proTips: 'Always place Batch Normalization *before* your activation function (like ReLU) and *after* linear operations (like Conv2D). This keeps your activation input scales healthy and predictable.',
  forwardEquation: '\\hat{x} = \\frac{x - \\mu_B}{\\sqrt{\\sigma_B^2 + \\epsilon}}, \\quad y = \\gamma \\widehat{x} + \\beta',
  derivativeEquation: '\\text{Propagates derivative through mean and variance formulas to adapt gamma and beta}',
  sizeFormulaHTML: 'H_{out} = H_{in}, \\quad W_{out} = W_{in}, \\quad C_{out} = C_{in}',
  parameterFormula: '\\text{Parameters} = 4 \\cdot C \\quad (\\gamma, \\beta, \\text{static mean}, \\text{static variance})',
  flopFormula: '\\text{FLOPs} = 4 \\cdot (C \\cdot H_{in} \\cdot W_{in}) \\quad (\\text{Shift, scale, normalize steps})',
...
export const InteractiveSimulator = () => {
  ...
  const bnMean = bnBatchInputs.reduce((a, b) => a + b, 0) / bnBatchInputs.length;
  const bnVariance = bnBatchInputs.reduce((a, b) => a + Math.pow(b - bnMean, 2), 0) / bnBatchInputs.length;
  const bnEps = 0.001;
    ...
    <div className="w-full flex flex-col lg:flex-row gap-8 items-center">
      <div className="flex-1 flex flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-mono text-zinc-400 uppercase tracking-widest">Covariate Shift Stabilizer</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-center text-[10.5px]">
          <div className="bg-zinc-900/30 p-2 rounded-lg border border-zinc-850">
            ...
            <input 
              ...
              onChange={(e) => setBnParams(p => ({ ...p, gamma: Number(e.target.value) }))}
          ...
          <div className="bg-zinc-900/30 p-2 rounded-lg border border-zinc-850">
            ...
            <input 
              ...
              step="0.1" 
              value={bnParams.beta} 
              onChange={(e) => setBnParams(p => ({ ...p, beta: Number(e.target.value) }))}
              className="w-16 bg-zinc-950 border border-zinc-850 rounded text-center p-0.5 mt-1 font-mono text-cyan-400 font-bold cursor-pointer"
            />
          </div>
        </div>
        ...
        <p className="text-[10.5px] leading-relaxed text-zinc-500">
          Input Batch values: <strong className="text-zinc-400">[ {bnBatchInputs.join(', ')} ]</strong>. Computes live batch parameters: Mean = <strong className="text-cyan-400 font-mono">{bnMean.toFixed(1)}</strong>, Variance = <strong className="text-cyan-400 font-mono">{bnVariance.toFixed(1)}</strong>.
        </p>
      </div>
      ...
      <div className="w-56 h-48 border border-zinc-850 p-3.5 rounded-xl bg-zinc-950 flex flex-col justify-center gap-1.5 shrink-0 font-mono text-[9px]">
        <span className="text-zinc-500 uppercase font-bold tracking-wide border-b border-zinc-900 pb-1.5 block">Step-By-Step Scaling Output</span>
        {bnBatchInputs.map((val, idx) => {
          const normalized = bnNormalizeValue(val);
          return (
            <div key={idx} className="flex justify-between items-center text-[10px] bg-zinc-900/40 p-1 px-1.5 rounded border border-zinc-900/40">
              <span className="text-zinc-350 font-bold">Input {val} {"->"}</span>
              <span className="text-emerald-400 font-bold">Norm: {normalized.toFixed(2)}</span>
            </div>
          );
        })}
      </div>

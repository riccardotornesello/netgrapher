...
const description: LayerDescription = {
  ...
  concept:
    "Extends 2D convolutions by adding a depth dimension. Moves a 3D filter volume along three axes (width, height, and depth/time) to detect voxel patterns or temporal progression cues.",
  keyTakeaways: [
  ...
  proTips:
    '3D convolutions are computationally intensive! Consider using "Pseudo-3D" convolutions (Conv 2D + Conv 1D) to decouple spatial parsing from temporal scanning, reducing parameters by up to 50%.',
  forwardEquation:
    "y_{c, d, i, j} = b_c + \\sum_{m} \\sum_{z,h,w} w_{c, m, z, h, w} \\cdot x_{m, d \\cdot s + z, i \\cdot s + h, j \\cdot s + w}",
  derivativeEquation:
    "\\text{Gradients are computed via 3D spatial convolutions on } \\partial L / \\partial y",
  sizeFormulaHTML:
    "D_{out} = \\lfloor\\frac{D_{in} + 2P - K}{S}\\rfloor + 1, \\quad H_{out} = \\lfloor\\frac{H_{in} + 2P - K}{S}\\rfloor + 1, \\quad W_{out} = \\lfloor\\frac{W_{in} + 2P - K}{S}\\rfloor + 1",
  parameterFormula:
    "\\text{Parameters} = (K \\cdot K \\cdot K \\cdot C_{in}) \\cdot C_{out} + C_{out}",
  flopFormula:
    "\\text{FLOPs} = 2 \\cdot D_{out} \\cdot H_{out} \\cdot W_{out} \\cdot C_{out} \\cdot (K \\cdot K \\cdot K \\cdot C_{in})",

...
const description: LayerDescription = {
  ...
  concept:
    "Reshapes multi-dimensional tensors (e.g., channels × height × width) into unified, linear 1D dense vectors. This establishes compatibility between convolutional and fully-connected layer inputs.",
  keyTakeaways: [
  ...
  proTips:
    "Always make sure you compute the flat dimension correctly. For a feature map of shape (C, H, W) entering Flatten, the output vector length is C × H × W.",
  forwardEquation:
    "y_{\\text{index}} = x_{c, h, w} \\quad \\text{where } \\text{index} = c \\cdot (H \\cdot W) + h \\cdot W + w",
  derivativeEquation:
    "\\frac{\\partial L}{\\partial x_{c,h,w}} = \\frac{\\partial L}{\\partial y_{\\text{index}}} \\quad (\\text{Direct identity derivative forwarding})",
  sizeFormulaHTML: "\\text{Vector Length} = C \\cdot D \\cdot H \\cdot W",
  parameterFormula: "\\text{Parameters} = 0",
  flopFormula:
    "\\text{FLOPs} = 0 \\quad (\\text{Only modifies memory indexing view})",
    ...
    <div className="w-full flex flex-col lg:flex-row gap-8 items-center">
      <div className="flex-1 flex flex-col gap-4">
        ...
        <p className="text-[11.5px] leading-relaxed text-zinc-400">
          Collapses multi-dimensional configurations (like heights, widths,
          coordinate planes) into a unified contiguous sequence. Click below to
          experience matrix flattening:
        </p>
        ...
        >
          {isFlattened
            ? "Reset Structure to 2D Output"
            : "Fold Layer: Flatten into 1D Vector"}
        </button>
      </div>
            ...
            <div className="grid grid-cols-3 gap-1 p-1.5 border border-zinc-800 rounded bg-zinc-900">
              {flattenSourceMap.map((cell, idx) => (
                <div
                  key={idx}
                  className={cn(
                    "w-7.5 h-7.5 rounded flex items-center justify-center font-bold text-[10px]",
                    cell.color,
                  )}
                >
                  {cell.val}
            ...
            <div className="flex flex-wrap gap-1 p-1 px-2 border border-cyan-800/40 rounded bg-cyan-950/10 justify-center">
              {flattenSourceMap.map((cell, idx) => (
                <div
                  key={idx}
                  className={cn(
                    "w-6 h-6 rounded flex items-center justify-center font-bold text-[9px] animate-scale-in",
                    cell.color,
                  )}
                >
                  {cell.val}

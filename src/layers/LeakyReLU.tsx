...
export const info: LayerDescription = {
  ...
  category: 'Activation',
  concept: 'A robust variation of classical ReLU. Instead of resetting all negative coordinates directly to zero, Leaky ReLU allows a very small, non-zero slope (typically 0.01) when x < 0. This keeps neurons slightly active even when negative.',
  keyTakeaways: [
  ...
  proTips: 'The negative slope parameter (alpha) is typically set to 0.01. If you want the network to learn this slope dynamically, use a Parametric ReLU (PReLU) layer instead!',
  forwardEquation: 'f(x) = \\max(\\alpha x, x) \\quad \\text{where } \\alpha \\approx 0.01',
  derivativeEquation: 'f\'(x) = \\begin{cases} 1 & x > 0 \\\\ \\alpha & x \\le 0 \\end{cases}',
